...
import ChildWedding from '@/models/ChildWedding';
  ...
  switch (method) {
    case 'GET':
      try {
        ...
        const data = await ChildWedding.findOne({ userId });
    ...
    case 'POST':
      try {
        ...
        const updated = await ChildWedding.findOneAndUpdate(
          { userId: data.userId },
          { $set: data },
    ...
    case 'DELETE':
      try {
        const { userId } = query;
        if (!userId) return res.status(400).json({ error: 'User ID is required' });
        await ChildWedding.findOneAndDelete({ userId });
